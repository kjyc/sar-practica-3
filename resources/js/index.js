document.addEventListener("DOMContentLoaded", function (event) {
            ...
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                if (xmlhttp.responseText == 'success') {
                    ...
                    if (!document.getElementById('si-error')) {
                        inputSiPassword.parentElement.after(createSiErrorMessage('si-error', 'Correo o contraseña incorrecta.'));
                    }
                }
            }
        }
    ...
    function createSiErrorMessage(id, message) {
    ...
    function verifyInput(element, id, message, isOk) {
        ...
        if (isOk) {
            ...
            if (!errorMessage) {
                element.parentElement.after(createSuErrorMessage(id, message));
            }
        }
    }

    function createSuErrorMessage(id, message) {
        let div = document.createElement('div');
        let p = document.createElement('p');
        div.setAttribute('id', id);
        div.setAttribute('class', 'row');
